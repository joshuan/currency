...
test('currencies keyboard navigation', async ({ page }) => {
	...
	const searchInput = page.locator('input[placeholder="Search currency or country"]');
	...
	const firstCheckbox = page.locator('.Config__Currency_Item input[type="checkbox"]').first();
	await expect(firstCheckbox).toBeFocused();

	// Press ArrowDown
	await page.keyboard.press('ArrowDown');
	const secondCheckbox = page.locator('.Config__Currency_Item input[type="checkbox"]').nth(1);
